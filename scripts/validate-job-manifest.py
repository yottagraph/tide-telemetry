...
VALID_RUNNERS = ("cloud_run", "k8s_job")
...
# Renamed 2026-05-23 as part of the Phase A K8s-Jobs-on-per-tenant-GKE
# pivot (see broadchurch's `.cursor/plans/eng_527_phase_a_substrate.plan.md`).
...
def validate(raw: Any, default_name: str) -> tuple[dict[str, Any], ValidationErrors]:
    ...
    # provisioning_model — only meaningful for k8s_job (selects a Spot
    # nodepool); Cloud Run Jobs don't expose a spot tier.
    ...
    if runner == "cloud_run" and provisioning_model != "standard":
        errs.add(
            "provisioning_model",
            "Cloud Run Jobs don't support Spot instances yet; only the "
            "`k8s_job` runner can set `provisioning_model: spot`",
        )
